...
SELECT P.projectnaam, M.voornaam || ' ' || M.achternaam as naam_verantwoordelijke, P.startdatum, P.einddatum, Q.uren AS geschat_aantal_uren , IJ.rate AS geschatte_rate, Q.uren * IJ.rate AS geschatte_kost_uren, Q.onkosten AS geschatte_onkosten, SUM(RTP.aantal_uren) AS reëel_aantal_uren, CAST(AVG(RT.rate) AS NUMERIC(4,2)) AS reële_rate, CAST(SUM(RTP.aantal_uren) * AVG(RT.rate) AS NUMERIC(10,2)) AS reële_kost_uren, SUM(RTP.kosten) AS reëele_onkosten
FROM r0932517_groep.project AS P 
INNER JOIN r0932517_groep.taak AS T 
			INNER JOIN r0932517_groep.rate_taak AS RT ON T.rate_id = RT.rate_id
			LEFT OUTER JOIN r0932517_groep.registreerder_taakprestaties AS RTP ON T.taak_id = RTP.taak_id
		ON P.id = T.id
INNER JOIN r0932517_groep.medewerker AS M ON M.personeelsnummer = P.verantwoordelijke_medewerker
INNER JOIN r0932517_groep.rate_algemene_werking AS IJ ON P.inschattingsjaar = IJ.inschattingsjaar
INNER JOIN
( SELECT P.id AS projectid, SUM(T.geschat_aantal_uren) AS uren, SUM(T.geschatte_onkosten) AS onkosten
FROM r0932517_groep.project AS P 
INNER JOIN r0932517_groep.taak AS T ON P.id = T.id
GROUP BY P.id
) AS Q ON Q.projectid = P.id
WHERE P.status = 'planned and ongoing'
GROUP BY P.id, Q.uren, Q.onkosten, M.personeelsnummer, IJ.rate
ORDER BY 4
